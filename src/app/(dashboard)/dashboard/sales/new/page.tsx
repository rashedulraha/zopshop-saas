...
export default function NewSalePage() {
  ...
  const handleCellChange = (rowId: string, field: "quantity" | "unitPrice" | "discount" | "vat", value: number) => {
    setItems(items.map(item => {
      if (item.id === rowId) {
        return {
          ...item,
  ...
  useEffect(() => {
    ...
    const calculatedGrandTotal = subtotalAccumulator - discountAccumulator + vatAccumulator + (Number(transportCost) || 0);
    ...
    <div className="flex flex-col gap-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div className="flex items-center gap-3">
          <Link href="/dashboard/sales" className="p-1.5 hover:bg-muted border border-border rounded-md text-muted-foreground hover:text-foreground transition-colors shrink-0">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <h1 className="text-3xl font-semibold text-foreground tracking-tight">New Sale</h1>
            <p className="text-muted-foreground mt-1 text-sm">Create a new customer sales invoice</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="border border-border bg-card p-5 rounded-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            {/* Sales Date */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Sales Date</label>
              <div className="relative">
        ...
        <div className="border border-border bg-card rounded-md overflow-hidden">
          <div className="px-4 py-3 border-b border-border bg-muted/20 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-foreground tracking-tight">Invoice Items list</h3>
            <button
              type="button"
              onClick={addRow}
            ...
            <table className="w-full text-sm text-left table-fixed min-w-[750px]">
              <thead className="text-xs text-muted-foreground uppercase bg-muted/10 border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-2 font-medium w-[15%]">Unit Price ($)</th>
                    ...
                    <td className="px-3 py-2">
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => handleCellChange(item.id, "unitPrice", parseFloat(e.target.value) || 0)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="border border-border bg-card p-5 rounded-md flex flex-col gap-4">
            <div className="border-b border-border pb-3 mb-1">
              <h3 className="text-sm font-semibold text-foreground tracking-tight">Payment Settlement</h3>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Due Amount ($)</label>
              <div className="relative">
                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <input
                  type="number"

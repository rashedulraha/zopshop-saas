...
const initialRolePerms: RolePermission[] = [
  { role: "Super Admin", permissions: createFullPerms(true) },
];

export default function PermissionsPage() {
  const [rolePerms, setRolePerms] =
    useState<RolePermission[]>(initialRolePerms);
  const [selectedRole, setSelectedRole] = useState<string>(
    initialRolePerms[0].role,
  );
  const [saved, setSaved] = useState(false);

  const currentRole = useMemo(
    () => rolePerms.find((r) => r.role === selectedRole) || rolePerms[0],
  ...
  const totalEnabled = Object.values(currentRole.permissions).filter(
    Boolean,
  ).length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Configure granular module-level permissions for each assigned role
          </p>
        </div>
        <button
          onClick={handleSave}

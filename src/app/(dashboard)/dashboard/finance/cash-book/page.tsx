...
import { Search, DollarSign, CheckCircle, TrendingDown, TrendingUp, HelpCircle, Receipt } from "lucide-react";
...
const initialCashBook: CashRecord[] = [
  { id: "TXN-001", date: "13 Oct 2026", type: "Debit", reference: "INV-1001", description: "Customer invoice checkout payment", amount: 299, balance: 4500 },
  { id: "TXN-002", date: "12 Oct 2026", type: "Credit", reference: "PAY-1100", description: "Paid outstanding to Jane Smith", amount: 3800, balance: 4201 },
  { id: "TXN-003", date: "11 Oct 2026", type: "Debit", reference: "INC-809", description: "Rental deposit refund from sub-agent", amount: 1500, balance: 8001 }
];
...
export default function CashBookPage() {
  ...
  const stats = useMemo(() => {
    const totalDebit = records.filter((r) => r.type === "Debit").reduce((sum, curr) => sum + curr.amount, 0);
    const totalCredit = records.filter((r) => r.type === "Credit").reduce((sum, curr) => sum + curr.amount, 0);
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          <h1 className="text-3xl font-semibold text-foreground tracking-tight">Cash Book</h1>
          <p className="text-muted-foreground mt-1 text-sm">Monitor daily physical cash flows, debits, credits, and cash in hand</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left table-fixed min-w-[650px] md:min-w-0">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/10 border-b border-border">
              <tr className="divide-x divide-border">
                <th className="px-5 py-3 font-semibold text-center w-[33%]">Cash in Hand</th>
                <th className="px-5 py-3 font-semibold text-center w-[33%]">Total Debits (Cash In)</th>
                <th className="px-5 py-3 font-semibold text-center w-[34%]">Total Credits (Cash Out)</th>
              </tr>
            </thead>
            <tbody>
              <tr className="divide-x divide-border">
          ...
          <table className="w-full text-xs text-left">
            <thead className="text-[10px] text-muted-foreground uppercase bg-muted/10 border-b border-border">
              <tr>
                ...
                <th className="px-4 py-2 font-semibold text-right">Cash Balance</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50">
              {filteredRecords.map((r) => (
                <tr key={r.id} className="hover:bg-muted/20 transition-colors">
                  <td className="px-4 py-3 font-semibold text-foreground font-mono">{r.id}</td>
                  <td className="px-4 py-3 text-muted-foreground">{r.date}</td>
                  <td className="px-4 py-3">
                    <span className={cn(
                      "inline-flex px-1.5 py-0.5 text-[10px] font-semibold border rounded",
                      r.type === "Debit" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : "bg-rose-500/10 text-rose-600 border-rose-500/20"
                    )}>
                      {r.type}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-semibold text-foreground font-mono">{r.reference}</td>
                  ...
                  <td className="px-4 py-3 font-bold text-foreground font-mono text-right">${r.balance.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>

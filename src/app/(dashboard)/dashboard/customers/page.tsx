...
import { 
  Users, Search, Plus, Phone, MapPin, Landmark, 
  ArrowUpRight, ArrowDownRight, DollarSign, Calendar,
  FileText, History, User, Receipt
} from "lucide-react";
...
const initialCustomers: Customer[] = [
  {
    ...
    totalPurchases: 4500,
    ledger: [
      { id: "L-1", date: "01 Oct 2026", type: "Opening Balance", reference: "-", debit: 1200, credit: 0, balance: 1200 },
      { id: "L-2", date: "05 Oct 2026", type: "Payment", reference: "TXN-9081", debit: 0, credit: 1200, balance: 0 },
      { id: "L-3", date: "13 Oct 2026", type: "Sale", reference: "INV-1001", debit: 299, credit: 0, balance: 299 }
    ],
    paymentHistory: [
      { id: "P-1", date: "05 Oct 2026", method: "Bkash", amount: 1200, reference: "TXN-9081" }
    ]
  },
  {
    id: "CUST-002",
    ...
    totalPurchases: 1800,
    ledger: [
      { id: "L-4", date: "01 Oct 2026", type: "Opening Balance", reference: "-", debit: 0, credit: 0, balance: 0 },
      { id: "L-5", date: "13 Oct 2026", type: "Sale", reference: "INV-1002", debit: 99, credit: 0, balance: 99 }
    ],
    paymentHistory: []
  },
  {
    ...
    totalPurchases: 3200,
    ledger: [
      { id: "L-6", date: "01 Oct 2026", type: "Opening Balance", reference: "-", debit: 500, credit: 0, balance: 500 },
      { id: "L-7", date: "08 Oct 2026", type: "Payment", reference: "TXN-8802", debit: 0, credit: 500, balance: 0 },
      { id: "L-8", date: "12 Oct 2026", type: "Sale", reference: "INV-1003", debit: 450, credit: 0, balance: 450 },
      { id: "L-9", date: "12 Oct 2026", type: "Payment", reference: "TXN-8812", debit: 0, credit: 450, balance: 0 }
    ],
    paymentHistory: [
      { id: "P-2", date: "08 Oct 2026", method: "Bank Transfer", amount: 500, reference: "TXN-8802" },
      { id: "P-3", date: "12 Oct 2026", method: "Cash", amount: 450, reference: "TXN-8812" }
    ]
  }
];
    ...
    <div className="flex flex-col gap-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Manage client details, ledger records, and payment history</p>
        </div>
        <button
      ...
      {isAddOpen ? (
        ...
        <div className="border border-border bg-card rounded-md max-w-2xl p-6">
          <div className="border-b border-border pb-4 mb-6">
            ...
            <p className="text-sm text-muted-foreground">Register a new customer profile in the system.</p>
          </div>
          <form onSubmit={handleAddCustomer} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Previous / Opening Due ($)</label>
                <div className="relative">
              ...
              {filteredCustomers.map((cust) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground mt-0.5">{cust.phone}</span>
                    <div className="flex items-center justify-between w-full mt-2 text-xs">
                      <span className="text-muted-foreground">Due Amount</span>
                      <span className={cn(
                        "font-semibold",
                        cust.currentDue > 0 ? "text-amber-500" : "text-emerald-500"
                      )}>${cust.currentDue.toFixed(2)}</span>
                    </div>
                  </button>
                );
              })}
              {filteredCustomers.length === 0 && (
                <span className="text-sm text-muted-foreground text-center py-6">No customers found</span>
              )}
            </div>
          </div>
            ...
            <div className="border border-border bg-card p-5 rounded-md flex flex-col">
              <div className="flex items-center gap-3 border-b border-border pb-4 mb-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">ID: {selectedCustomer.id}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-border mt-5 pt-5">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Previous Due</span>
                  <span className="text-lg font-semibold text-foreground mt-0.5">${selectedCustomer.previousDue.toFixed(2)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Current Due</span>
                  <span className={cn(
                    "text-lg font-semibold mt-0.5",
                    selectedCustomer.currentDue > 0 ? "text-amber-500" : "text-emerald-500"
                  )}>${selectedCustomer.currentDue.toFixed(2)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Purchases</span>
                  <span className="text-lg font-semibold text-primary mt-0.5">${selectedCustomer.totalPurchases.toFixed(2)}</span>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-muted-foreground uppercase bg-muted/10 border-b border-border sticky top-0 z-10 bg-muted/95 backdrop-blur-sm shadow-sm">
                    <tr>
                      ...
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap">Debit (Sale)</th>
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap">Credit (Pay)</th>
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap text-right">Balance</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={entry.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-1.5 whitespace-nowrap">
                          <span className={cn(
                            ...
                            entry.type === "Sale" ? "bg-amber-500/10 text-amber-500" :
                        ...
                        <td className="px-4 py-1.5 font-medium text-foreground whitespace-nowrap">{entry.debit > 0 ? `$${entry.debit.toFixed(2)}` : "-"}</td>
                        <td className="px-4 py-1.5 font-medium text-emerald-500 whitespace-nowrap">{entry.credit > 0 ? `$${entry.credit.toFixed(2)}` : "-"}</td>
            ...
            <div className="border border-border bg-card rounded-md overflow-hidden">
              ...
              <div className="overflow-x-auto max-h-[220px] overflow-y-auto custom-scrollbar">
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-muted-foreground uppercase bg-muted/10 border-b border-border sticky top-0 z-10 bg-muted/95 backdrop-blur-sm shadow-sm">
                    <tr>
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap">Receipt Date</th>
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap">Payment Method</th>
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap">Txn Reference</th>
                      <th className="px-4 py-1.5 font-medium whitespace-nowrap text-right">Amount Received</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/50">
                    {selectedCustomer.paymentHistory.map((pmt) => (
                      <tr key={pmt.id} className="hover:bg-muted/30 transition-colors">
                        <td className="px-4 py-1.5 whitespace-nowrap text-muted-foreground">{pmt.date}</td>
                        <td className="px-4 py-1.5 whitespace-nowrap font-medium text-foreground">{pmt.method}</td>
                        <td className="px-4 py-1.5 font-semibold text-foreground whitespace-nowrap">{pmt.reference}</td>
                        <td className="px-4 py-1.5 font-semibold text-emerald-500 whitespace-nowrap text-right">${pmt.amount.toFixed(2)}</td>
                      </tr>
                    ))}
